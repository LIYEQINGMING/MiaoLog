Create Codex pet row `running-right` for `guga`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A rounder and cuter chibi penguin hoodie girl pet with a softer face and friendlier proportions. Preserve silhouette, face, palette, material, proportions, markings, and props.
