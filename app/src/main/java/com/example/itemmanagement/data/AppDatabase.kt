...
import com.example.itemmanagement.data.dao.TagGroupDao
import com.example.itemmanagement.data.dao.unified.GalleryDao
...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
            ...
            return             INSTANCE ?: synchronized(this) {
                ...
                .addCallback(object : RoomDatabase.Callback() {
                    override fun onCreate(db: SupportSQLiteDatabase) {
                        ...
                        try {
                            // 插入状态定义
                            android.util.Log.e("TemplateDB", "步骤1: 插入基础状态定义...")
                            db.execSQL("INSERT INTO `item_status_definitions` (`id`, `name`, `isSystemBuiltIn`) VALUES (1, '服役中', 1)")
                            db.execSQL("INSERT INTO `item_status_definitions` (`id`, `name`, `isSystemBuiltIn`) VALUES (2, '未购买', 1)")
                            db.execSQL("INSERT INTO `item_status_definitions` (`id`, `name`, `isSystemBuiltIn`) VALUES (3, '已退役', 1)")
                            db.execSQL("INSERT INTO `item_status_definitions` (`id`, `name`, `isSystemBuiltIn`) VALUES (4, '已过期', 1)")
                            db.execSQL("INSERT INTO `item_status_definitions` (`id`, `name`, `isSystemBuiltIn`) VALUES (5, '待补款', 1)")
                            
                            // 插入默认的通用模板
                            android.util.Log.e("TemplateDB", "步骤2: 插入通用模板...")
        ...
        private val MIGRATION_52_53 = object : Migration(52, 53) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // 1. 货币单位
                database.execSQL("ALTER TABLE unified_items ADD COLUMN currencyCode TEXT NOT NULL DEFAULT 'CNY'")
                // 2. 统计控制
                database.execSQL("ALTER TABLE unified_items ADD COLUMN excludeFromTotalValue INTEGER NOT NULL DEFAULT 0")
                database.execSQL("ALTER TABLE unified_items ADD COLUMN excludeFromTotalCount INTEGER NOT NULL DEFAULT 0")
                // 3. 订阅制属性
                database.execSQL("ALTER TABLE unified_items ADD COLUMN isSubscription INTEGER NOT NULL DEFAULT 0")
                database.execSQL("ALTER TABLE unified_items ADD COLUMN autoRenew INTEGER NOT NULL DEFAULT 0")
                database.execSQL("ALTER TABLE unified_items ADD COLUMN subscriptionCycle TEXT DEFAULT NULL")
                // 4. 模板关联
                database.execSQL("ALTER TABLE unified_items ADD COLUMN templateId INTEGER DEFAULT NULL")
                
                // 5. 创建相关索引优化查询
                database.execSQL("CREATE INDEX IF NOT EXISTS `index_unified_items_isSubscription` ON `unified_items` (`isSubscription`)")
                database.execSQL("CREATE INDEX IF NOT EXISTS `index_unified_items_templateId` ON `unified_items` (`templateId`)")
            }
        }
        ...
        private val MIGRATION_53_54 = object : Migration(53, 54) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // 1. 创建状态定义表
                database.execSQL("""
                    CREATE TABLE IF NOT EXISTS `item_status_definitions` (
                        `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                        `name` TEXT NOT NULL,
                        `isSystemBuiltIn` INTEGER NOT NULL,
                        `colorHex` TEXT,
                        `iconName` TEXT
                    )
                """)
                ...
                // 2. 插入内置基础状态
                database.execSQL("INSERT INTO `item_status_definitions` (`id`, `name`, `isSystemBuiltIn`) VALUES (1, '服役中', 1)")
                database.execSQL("INSERT INTO `item_status_definitions` (`id`, `name`, `isSystemBuiltIn`) VALUES (2, '未购买', 1)")
                database.execSQL("INSERT INTO `item_status_definitions` (`id`, `name`, `isSystemBuiltIn`) VALUES (3, '已退役', 1)")
                database.execSQL("INSERT INTO `item_status_definitions` (`id`, `name`, `isSystemBuiltIn`) VALUES (4, '已过期', 1)")
                database.execSQL("INSERT INTO `item_status_definitions` (`id`, `name`, `isSystemBuiltIn`) VALUES (5, '待补款', 1)")
                
                // 3. 修改 item_states 表：增加 statusId 列并迁移数据
                database.execSQL("ALTER TABLE `item_states` ADD COLUMN `statusId` INTEGER NOT NULL DEFAULT 1")
                
                // 映射枚举值到 statusId
                database.execSQL("UPDATE `item_states` SET `statusId` = 1 WHERE `stateType` = 'INVENTORY'")
                database.execSQL("UPDATE `item_states` SET `statusId` = 2 WHERE `stateType` = 'SHOPPING'")
                database.execSQL("UPDATE `item_states` SET `statusId` = 3 WHERE `stateType` = 'DELETED'")
                
                // 创建新索引
                database.execSQL("CREATE INDEX IF NOT EXISTS `index_item_states_statusId` ON `item_states` (`statusId`)")
                database.execSQL("CREATE INDEX IF NOT EXISTS `index_item_states_itemId_statusId_isActive` ON `item_states` (`itemId`, `statusId`, `isActive`)")
            }
        }

        private val MIGRATION_54_55 = object : Migration(54, 55) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // 1. 创建 custom_attribute_definitions 表
                database.execSQL("""
                ...
                database.execSQL("""
                    CREATE TABLE IF NOT EXISTS `item_custom_attributes` (
                        ...
                        FOREIGN KEY(`definitionId`) REFERENCES `custom_attribute_definitions`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE 
                    )
                """)
        ...
        private val MIGRATION_55_56 = object : Migration(55, 56) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                // 2. 创建 gallery_item_cross_ref 表
                database.execSQL("""
                    CREATE TABLE IF NOT EXISTS `gallery_item_cross_ref` (
                        `galleryId` INTEGER NOT NULL,
                        `itemId` INTEGER NOT NULL,
                        `displayOrder` INTEGER NOT NULL DEFAULT 0,
                        `customRemark` TEXT,
                        PRIMARY KEY(`galleryId`, `itemId`),
                        FOREIGN KEY(`galleryId`) REFERENCES `galleries`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE ,
                        FOREIGN KEY(`itemId`) REFERENCES `unified_items`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE 
                    )
                """)
                database.execSQL("CREATE INDEX IF NOT EXISTS `index_gallery_item_cross_ref_galleryId` ON `gallery_item_cross_ref` (`galleryId`)")
                database.execSQL("CREATE INDEX IF NOT EXISTS `index_gallery_item_cross_ref_itemId` ON `gallery_item_cross_ref` (`itemId`)")
                ...
                // 4. 修改 tags 表：增加 groupId 列
                database.execSQL("ALTER TABLE `tags` ADD COLUMN `groupId` INTEGER DEFAULT NULL")
                
                // 5. 修改 item_templates 表：增加新列
                database.execSQL("ALTER TABLE `item_templates` ADD COLUMN `description` TEXT DEFAULT NULL")
                database.execSQL("ALTER TABLE `item_templates` ADD COLUMN `icon` TEXT DEFAULT NULL")
                database.execSQL("ALTER TABLE `item_templates` ADD COLUMN `customAttributeIds` TEXT DEFAULT NULL")
                database.execSQL("ALTER TABLE `item_templates` ADD COLUMN `createdAt` INTEGER NOT NULL DEFAULT 0")
                database.execSQL("ALTER TABLE `item_templates` ADD COLUMN `isVisible` INTEGER NOT NULL DEFAULT 1")
                
                // 更新 createdAt 的默认值为当前时间（对旧数据）
                val currentTime = System.currentTimeMillis()
                database.execSQL("UPDATE `item_templates` SET `createdAt` = $currentTime WHERE `createdAt` = 0")
            }
        }

        private val MIGRATION_56_57 = object : Migration(56, 57) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // 为 custom_attribute_definitions 表添加新字段
                database.execSQL("ALTER TABLE `custom_attribute_definitions` ADD COLUMN `priceRole` TEXT DEFAULT NULL")
                database.execSQL("ALTER TABLE `custom_attribute_definitions` ADD COLUMN `supportedActions` TEXT DEFAULT NULL")
                database.execSQL("ALTER TABLE `custom_attribute_definitions` ADD COLUMN `supportsRecurrence` INTEGER DEFAULT NULL")
                database.execSQL("ALTER TABLE `custom_attribute_definitions` ADD COLUMN `defaultActions` TEXT DEFAULT NULL")
                database.execSQL("ALTER TABLE `custom_attribute_definitions` ADD COLUMN `defaultCurrency` TEXT DEFAULT NULL")

                // 为 item_custom_attributes 表添加新字段
                database.execSQL("ALTER TABLE `item_custom_attributes` ADD COLUMN `priceAmount` REAL DEFAULT NULL")
                database.execSQL("ALTER TABLE `item_custom_attributes` ADD COLUMN `priceCurrency` TEXT DEFAULT NULL")
                database.execSQL("ALTER TABLE `item_custom_attributes` ADD COLUMN `priceDate` INTEGER DEFAULT NULL")
                database.execSQL("ALTER TABLE `item_custom_attributes` ADD COLUMN `includeInAverage` INTEGER DEFAULT NULL")
                database.execSQL("ALTER TABLE `item_custom_attributes` ADD COLUMN `includeInDailyValue` INTEGER DEFAULT NULL")
                database.execSQL("ALTER TABLE `item_custom_attributes` ADD COLUMN `isRecurring` INTEGER DEFAULT NULL")
        ...
        val MIGRATION_63_64 = object : Migration(63, 64) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "ALTER TABLE `rule_definitions` ADD COLUMN `activationMode` TEXT NOT NULL DEFAULT 'ALWAYS_ON'"
                )
                database.execSQL(
                    "ALTER TABLE `rule_definitions` ADD COLUMN `toggleUiJson` TEXT DEFAULT NULL"
                )
                database.execSQL(
                    "ALTER TABLE `item_rule_bindings` ADD COLUMN `isEnabled` INTEGER NOT NULL DEFAULT 1"
                )
            }
        }
        ...
        val MIGRATION_64_65 = object : Migration(64, 65) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "ALTER TABLE `item_rule_bindings` ADD COLUMN `togglePlacementSlotKey` TEXT DEFAULT NULL"
                )
            }
        }
        ...
        val MIGRATION_65_66 = object : Migration(65, 66) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "ALTER TABLE `rule_definitions` ADD COLUMN `scheduleConfigJson` TEXT DEFAULT NULL"
                )
                database.execSQL(
                    "ALTER TABLE `rule_definitions` ADD COLUMN `scheduleEventConfigJson` TEXT DEFAULT NULL"
                )
                database.execSQL(
                    "ALTER TABLE `item_rule_bindings` ADD COLUMN `runtimeStateJson` TEXT DEFAULT NULL"
                )
            }
        }
        ...
        val MIGRATION_66_67 = object : Migration(66, 67) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "ALTER TABLE `rule_definitions` ADD COLUMN `canvasDefinitionJson` TEXT DEFAULT NULL"
                )
            }
        }
    }
