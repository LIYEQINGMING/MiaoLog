# 属性管理开发日志
## 20260626 10:45 第十一轮开发结果
### 完成内容
已完成

- 物品属性主链路已切到新属性定义体系，旧的 CustomAttributeDefinitionEntity 不再作为新增/编辑页的定义来源，核心仓储改为读取新属性定义： UnifiedItemRepository.kt
- 物品属性值实体已改为使用新属性 String id 存储，并同步调整 DAO 与数据库版本迁移到 60 ： ItemCustomAttributeEntity.kt ItemCustomAttributeDao.kt AppDatabase.kt
- 新增物品保存逻辑已改为按新属性字段名 custom_{attributeId} 构建属性值，并兼容价格属性的币种与计入总价开关： AddItemViewModel.kt
- 编辑物品回填与保存逻辑也已切到新属性定义，支持按新属性类型回填文本、数字、日期、布尔和价格值： EditItemViewModel.kt
- 物品表单组件已整体替换到新属性定义渲染，字段命名、类型判断、价格属性显示、函数区联动都已同步调整： ItemFormComponents.kt
- 新增页、编辑页、模板应用入口都已改为传递新属性定义，不再走旧自定义属性定义类型： AddItemFragment.kt EditItemFragment.kt AddItemScreen.kt EditItemScreen.kt

## 20260625 14:48 第十轮开发结果
### 完成内容
已完成

- 属性管理模块这轮我已经按“先把属性体系做完整，再回头碰物品模型”的方向一口气推进完了，核心改动集中在 AttributeManagementScreen.kt 、 AttributeManagementViewModel.kt 、 AttributeManagementUiModels.kt 和 AttributeFragment.kt 。
- 我没有按你的要求主动编译，只做了 IDE 级诊断检查；当前上述关键文件诊断为空。
这轮补齐的能力

- 属性管理从“列表/详情骨架”补成了完整流：支持新建属性、编辑属性、删除属性、从属性模板创建属性、查看属性详情。
- 规则管理从“只有列表和详情”补成了完整流：支持新建规则、编辑规则、删除规则、从规则模板创建规则、查看规则详情。
- 模板管理不再只是浏览：属性模板详情可以直接进入“从模板创建属性”，规则模板详情可以直接进入“从模板创建规则”。
- 属性编辑页补上了完整字段：名称、图标、说明、值类型、选项来源、输入方式、多值、固定选项、规则绑定。
- 规则编辑页补上了完整字段：名称、说明、规则类型、输入角色、必需依赖、可选依赖、输出定义、表达式说明。
结构调整

- UI 路由状态已重构为真正的页面流：列表、属性详情、规则详情、模板详情、属性编辑、规则编辑，见 AttributeManagementUiModels.kt 。
- 属性页、规则页、模板页现在都走统一的页内导航和操作模型，不再靠之前那套“模拟保存”的占位逻辑。
- 规则标签页的主操作按钮已启用，现在和属性/模板页一样都有明确的创建入口。
- 属性卡片、规则卡片都补了列表级菜单操作，自定义项支持直接编辑和删除，不必必须先进详情页。
数据与保存

- 属性保存现在会真实落成 AttributeDefinition ，并按编辑/新建区分保留或生成 id / key 。
- 规则保存现在会真实落成 RuleDefinition ，并按编辑/新建区分保留或生成 id / key 。
- 新建属性 ID 统一走 attr_${UUID} ，新建规则 ID 统一走 rule_${UUID} ，保持唯一且不会重复。
- 保存时加了基础校验：名称不能为空、固定选项属性必须有选项、规则至少要有输入角色和输出定义、同名属性/规则会阻止保存。
- 模板统计从静态缓存改成了按当前状态实时生成，避免模板详情或模板列表里的计数一直停留在旧值，见 AttributeManagementViewModel.kt 。
当前边界

- 我这轮刻意没有去碰 Item 主模型、物品表单属性值落地、规则执行引擎这些部分，仍然遵守我们前面定的边界。
- 规则现在是“定义和管理”做完了，但“真正执行规则并回写物品结果”还没接，这一步应该放在后续物品模型改造阶段一起做。
- 属性使用数量、规则引用数量目前还是管理层占位统计口径，还没连到物品实例层的真实计数。
...
- 创建新实体 ：新建了 AttributeDefinitionEntity ，该实体用于承接属性定义的核心字段，并将复杂对象（如下拉选项、规则绑定关系）映射为 JSON 字符串，以保证结构轻量。后续需继续按新规范收敛为值类型、值属性、值来源、交互方式等结构。
...
- 属性视图 ：展示属性来源、值类型、值来源、值属性摘要、模板来源、规则摘要、使用数量。
